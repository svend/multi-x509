...
impl<R: BufRead> Iterator for Certs<R> {
    type Item = String;

    fn next(&mut self) -> Option<String> {
        loop {
            let mut line = String::new();
            let num_bytes = self.r.read_line(&mut line).unwrap();
            if num_bytes == 0 {
                break;
            }
...
mod tests {
    ...
    const CERTS: &str = r#"-----BEGIN CERTIFICATE-----
...
"#;
    ...
    const CERT_0: &str = r#"-----BEGIN CERTIFICATE-----

...
"#;
    ...
    fn test_certs() {
        ...
        assert_eq!(certs.next(), Some(CERT_1.to_string()));
        assert_eq!(certs.next(), Some(CERT_2.to_string()));
        assert_eq!(certs.next(), None);
    }
}
